 ...
 * limita con {@link Pageable} (más recientes primero).
 * <p>
...
public interface AuditLogRepository extends Repository<AuditLogEntry, UUID> {
    ...
    List<AuditLogEntry> findByProjectIdOrderByOccurredAtDescIdDesc(UUID projectId, Pageable pageable);

    @Query("SELECT e FROM AuditLogEntry e "
            + "WHERE e.projectId = :projectId AND e.occurredAt >= :since "
            + "ORDER BY e.occurredAt DESC, e.id DESC")
    List<AuditLogEntry> findByProjectAndSince(@Param("projectId") UUID projectId,
