...
/** Traduce las excepciones del módulo {@code vault} a RFC 7807 con {@code code}. */
@RestControllerAdvice(basePackageClasses = ProjectVaultController.class)
class VaultExceptionHandler {

    @ExceptionHandler(VaultSecretNotFoundException.class)
    ResponseEntity<ProblemDetail> handleNotFound(VaultSecretNotFoundException exception) {
        return notFound(exception.getMessage());
    }

    @ExceptionHandler(VaultSecretAlreadyExistsException.class)
    ResponseEntity<ProblemDetail> handleConflict(VaultSecretAlreadyExistsException exception) {
