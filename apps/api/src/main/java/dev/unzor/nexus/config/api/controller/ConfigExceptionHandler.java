...
/** Traduce las excepciones del módulo {@code config} a RFC 7807 con {@code code}. */
@RestControllerAdvice(basePackageClasses = ProjectConfigController.class)
class ConfigExceptionHandler {

    @ExceptionHandler(ConfigKeyNotFoundException.class)
    ResponseEntity<ProblemDetail> handleNotFound(ConfigKeyNotFoundException exception) {
        return notFound(exception.getMessage());
    }

    @ExceptionHandler(InvalidConfigValueException.class)
    ResponseEntity<ProblemDetail> handleInvalidValue(InvalidConfigValueException exception) {
