...
export function DocumentsModule() {
  ...
  const renderM = useRenderDocumentTemplate(projectId);
  ...
  const busy = createM.isPending || updateM.isPending || deleteM.isPending || renderM.isPending;
  const fieldErrors = toFieldErrors(createM.error ?? updateM.error);
  const actionError =
    createM.error ?? updateM.error ?? deleteM.error ?? renderM.error
      ? toMessage(
          createM.error ?? updateM.error ?? deleteM.error ?? renderM.error,
          DOCUMENTS_MESSAGES,
        )
  ...
  function openRender(template: DocumentTemplate) {
    resetActionErrors();
    renderM.reset();
  ...
  async function doRender() {
    ...
    try {
      const parsed = renderVars.trim() ? JSON.parse(renderVars) : {};
    ...
    resetActionErrors();
    try {
      const result = await renderM.mutateAsync({
        templateId: renderTarget.id,
        variables,
      });
      setRenderOutput(result.output);
    } catch {
      setRenderOutput(null);
      /* se muestra vía renderM.error */
    }
  }
  ...
  return (
    <Stagger className="grid flex-1 grid-cols-1 gap-6">
      ...
      >
        {templates.length === 0 ? (
          <EmptyState
            ...
            description="Create your first template, e.g. a welcome body with {$name}."
          />
        ) : (
          <Table>
            <TableHeader>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Render {renderTarget?.name}</DialogTitle>
            <DialogDescription>
              Supply variables as a JSON object. Triggers like{" "}
              <code className="font-mono">{"{$name}"}</code> are substituted.
              The render is saved to history.
            </DialogDescription>
          </DialogHeader>
          <div className="flex flex-col gap-4 py-1">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="doc-render-vars">Variables (JSON)</Label>
              <Textarea
                id="doc-render-vars"
                value={renderVars}
          ...
          <DialogFooter>
            ...
            <Button onClick={doRender} disabled={busy || !renderTarget}>
